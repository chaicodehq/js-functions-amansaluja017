...
export function compose(...fns) {
  ...
  return function (args) {
    return fns.reduceRight((acc, curr) => curr(acc), args)
  }
}
...
export function createRecipe(steps) {
  // Your code here
  const stepsName = ["grind", "roast", "mix", "pack"];
  
  return function (args) {
    if (!Array.isArray(steps) || !steps.length) return args
    
    return steps.reduce((acc, curr) => {
      if (stepsName.includes(curr)) {
        if (curr === "grind") return grind(acc)
        if (curr === "roast") return roast(acc)
        if (curr === "mix") return mix(acc)
        if (curr === "pack") return pack(acc)
      }
